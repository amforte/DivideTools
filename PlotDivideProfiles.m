function PlotDivideProfiles(head_vals,StreamSegments,varargin)
    ...
    if isempty(chL)
		for ii=1:num_divides
			...
			f1=figure(fig_num);
			set(f1,'Units','inches','Position',[1.0 1.5 7.5 10],'renderer','painters','PaperSize',[8.5 11],'PaperPositionMode','auto');
			clf
			colormap(jet);
			...
			if ~isempty(name_prefix)
				print(f1,[name_prefix '_Divide_' num2str(ii) '.pdf'],'-dpdf');
			end
