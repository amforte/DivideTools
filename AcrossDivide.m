function [head_vals]=AcrossDivide(DEM,FD,DS_OUT,varargin)
    ...
    if sv_db
        if strcmp(outlet_method,'picked_outlets')
            head_valsN=double(vertcat(head_vals{:}));
            X=head_valsN(:,1); Y=head_valsN(:,2); Elev=head_valsN(:,3); Grad=head_valsN(:,4); Rlf=head_valsN(:,5); Chi=head_valsN(:,6); Div=head_valsN(:,7);
            % Create Geometry column
            Geometry=repmat({'Point'},numel(X),1);
            T=table(Geometry,X,Y,Elev,Grad,Rlf,Chi,Div);
            MS=table2struct(T);
